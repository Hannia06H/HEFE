...
<body>
    ...
    <article class="art-cent d-flex justify-content-center align-items-center">
        ...
        <form action="{{ route('modDom', ['id' => $dom->first()->id]) }}" method="post" enctype="multipart/form-data">
            @csrf
            @method('PUT')
                <div class="form-group row">
                    <label class="control-label col-12">Nombre</label>
                    <div class="col-12"> 
                    <input type="text" id="nombre" value="{{ $dom->first()->nombre }}" name="nombre" class="form-control">
                    </div>
                </div>
                <div class="form-group row">
                    <label class="control-label col-12">Apellido</label>
                    <div class="col-12">
                    <input type="text" value="{{ $dom->first()->apellido }}" name="apellido" class="form-control">
                    </div>
                </div>
                <div class="form-group row">
                    <label class="control-label col-12">Celular</label>
                    <div class="col-12">
                    <input type="text" value="{{ $dom->first()->celular }}" name="celular" class="form-control">
                    </div>
                </div>
                <div class="form-group row">
                    <label class="control-label col-12">Quien recibe?</label>
                    <div class="col-12">
                    <input type="text" value="{{ $dom->first()->recibe }}" name="recibe" class="form-control">
                    </div>
                </div>
                <div class="form-group row">
                    <label class="control-label col-12">Fecha de entrega</label>
                    <div class="col-12">
                        <input name="fecha"  value="{{ $dom->first()->fecha }}" type="date" />
                    </div>
                </div>
                <div class="form-group row">
                    <label class="control-label col-12">Direccion y referencia</label>
                    <div class="col-12">
                        <textarea name="referencia"  id="" cols="50" rows="5">{{ $dom->first()->referencia }}</textarea>
                    </div>
                </div>
                <br>
